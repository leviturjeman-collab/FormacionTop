...
Antes de desarrollar o impartir este módulo, leer [[../../../01_INVESTIGACION_OFICIAL/OpenAI_Academy_ChatGPT_Codex|OpenAI, ChatGPT, Codex, skills y workflows]]. Ese archivo contiene el desarrollo largo sobre Codex CLI, Codex cloud, tareas técnicas, skills, plugins, permisos, revisión y entornos.

## Objetivo

Enseñar a trabajar con Codex como agente técnico: leer contexto, planificar, editar, probar, revisar y entregar cambios.
...
El alumno sabe pedir una tarea técnica con definición de terminado y revisar el resultado antes de aceptarlo.

## Qué debe dominar el alumno

Codex debe enseñarse como colaborador técnico con capacidad de leer, modificar y verificar trabajo en un repositorio. Esto exige más rigor que una conversación normal. Una tarea de Codex necesita objetivo, contexto, límites, criterio de aceptación y verificación. El alumno debe aprender a pedir primero exploración cuando no entiende el proyecto, planificación cuando el cambio es grande, edición enfocada cuando el alcance está claro y revisión cuando ya existe un diff.

El bucle profesional es: entender el proyecto, acotar el cambio, planificar, editar, ejecutar pruebas, leer el diff, corregir, documentar y entregar. El alumno no debe aceptar cambios porque "Codex los hizo". Debe saber leer qué archivos cambiaron, por qué cambiaron y cómo se verificó el resultado. En clases tempranas, Codex puede trabajar en modo lectura: explicar estructura, detectar rutas relevantes o proponer plan sin modificar archivos. Después puede tocar cambios pequeños. Solo más adelante debe asumir tareas largas o cloud.

Codex CLI enseña trabajo local: abrir un repositorio, lanzar Codex, controlar permisos y ver comandos/diffs. Codex cloud enseña delegación en entornos reproducibles: conectar GitHub, configurar dependencias, variables, secretos, herramientas y revisar resultados de tareas paralelas o de larga duración. La diferencia entre ambas superficies debe explicarse con ejemplos reales.

## Práctica principal

El alumno debe completar tres tareas:

- Pedir a Codex que explique un proyecto sin editar.
- Pedir un cambio pequeño con tests.
- Pedir una revisión de diff con foco en bugs, riesgos y pruebas.

El error provocado será aceptar un cambio sin revisar. Después el alumno tendrá que encontrar una regresión o una prueba ausente y pedir una corrección precisa.
---
...
Para Resumen, la pregunta central es: que cambia en el trabajo del alumno cuando domina este tema. Si el tema es una fuente, debe aprender a usarla para no depender de opiniones genericas. Si es una leccion, debe aprender a explicar y aplicar el concepto. Si es un laboratorio, debe construir algo, romperlo y repararlo. Si es una evaluacion, debe demostrar competencia observable. Si es una plantilla, debe producir materiales consistentes. Si es un modulo, debe conectar varias lecciones en una progresion clara.
...
Un buen check para Resumen es pedir al alumno que lo explique en tres niveles. En 60 segundos debe dar una version simple. En modelo mental debe dibujar o describir las piezas. En profundidad profesional debe mencionar limites, riesgos, verificaciones y fuentes. Si no puede hacer los tres niveles, la leccion necesita mas trabajo antes de pasar al laboratorio.
...
- [[01_INVESTIGACION_OFICIAL/OpenAI_Academy_ChatGPT_Codex]]
...
# Capa AAAA+ - 00 Resumen

## Proposito profesional de esta nota

Esta nota queda elevada al estandar AAAA+. Su funcion dentro de la boveda es servir como pieza profesional de modulo didactico, explicacion profunda, practica guiada, laboratorio, evaluacion y fuentes. No debe leerse como una nota aislada ni como un resumen rapido. Debe poder ayudar a un alumno a entender el tema, aplicarlo a su trabajo, convertirlo en proyecto, evaluarlo y defenderlo con criterio.
...
No usar herramientas avanzadas por estetica. No introducir agentes, MCP, Docker, APIs o codigo si el objetivo se resuelve mejor con workflow, plantilla o skill simple. No tocar datos reales sin politica de privacidad y permisos.
...
Dominio: Codex y trabajo tecnico asistido.

Este archivo trabaja principalmente sobre: repositorios, tareas tecnicas, diffs, pruebas y skills.

El resultado esperado no es solo conocimiento. El resultado esperado es: tarea Codex con plan, diff revisado, tests y defensa.
...
# Capa ultra-premium AAAA+ - 00 Resumen
...
Prueba de dominio: alumno puede explicar, practicar y entregar evidencia.
...
Usar **Resumen** para producir el entregable definido en la metadata: **clase, practica y evaluacion**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Resumen**.
