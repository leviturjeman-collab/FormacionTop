...
let production = null
try { production=JSON.parse(await fs.readFile('docs/auditoria/verificacion-produccion.json','utf8')) } catch {}
const productionChecks=production?.publishedContent?.results?.flatMap(result=>result.checks)||[]
const productionVerified=production?.deployment?.status==='Ready'&&production?.accessUi?.checks?.length===7&&productionChecks.length===18&&productionChecks.every(check=>check.status===(check.path==='/'?200:401))
const productionSummary=productionVerified
  ? `Producción verificada: despliegue ${production.deployment.id}, commit ${production.deployment.applicationCommit}. UI real: 7 comprobaciones en ${production.accessUi.base}; 18 comprobaciones de acceso anónimo en 3 dominios, con portada 200 y rutas protegidas 401. Cobertura limitada a los escenarios registrados en verificacion-produccion.json; no certifica todas las interacciones ni automatizaciones de terceros.`
  : 'Producción pendiente de verificación; las pruebas de diseño y contenido de esta revisión corresponden a la web local.'
...
const finalViteLog=await fs.readFile('.temp/release-vite.log','utf8')
const bundleSize=/index-[^\s]+\.js\s+([\d.]+) kB[^\n]*gzip:\s+([\d.]+) kB/.exec(releaseLog)
const releaseEvidence={command:'npm test',status:releaseLog.includes('✓ built in')?'passed':'unconfirmed',store:23,sessionClient:8,sessionServer:9,clipboard:4,contentShards:Number(/PASS content shards: (\d+) assertions/.exec(releaseLog)?.[1] || 0),scope:'Release tests cover the local working tree. Supabase live checks and production verification are reported separately.',evidence:'.temp/release-tests.log',finalCssBuild:finalViteLog.includes('✓ built in')?'passed':'unconfirmed',finalCssBuildEvidence:'.temp/release-vite.log'}
...
el('production').textContent=data.productionSummary;
