...
try {
  ...
  const one = await rpc('academy_admin_create', [admin.token, { login: 'student-one', name: 'Student one' }, secret])
  const two = await rpc('academy_admin_create', [admin.token, { login: 'student-two', name: 'Student two' }, secret])
  check(one.id !== two.id, 'Same credential is safely disambiguated by unique identifier')
  await assert.rejects(() => rpc('academy_admin_create', [admin.token, { login: 'student-one', name: 'Duplicate' }, secret])); checks++
  const student = await rpc('academy_sign_in', ['student-one', secret])
  ...
  const secondSession = await rpc('academy_sign_in', ['student-two', secret])
  ...
  check(!(await rpc('academy_sign_in', ['student-one', secret])).ok, 'Suspended learner cannot log in')
  ...
  check(!(await rpc('academy_sign_in', ['student-one', secret])).ok, 'Old credential is invalid after reset')
  ...
  const legacy = (await db.query(`insert into public.learners(name,pin_hash,pin,pin_visible) values ('Legacy learner',extensions.crypt($1,extensions.gen_salt('bf',4)),$1,$1) returning id`, [secret])).rows[0].id
  ...
  check((await db.query('select pin_visible from public.learners where id=$1', [legacy])).rows[0].pin_visible === null, 'Visible credential copy cleared')
