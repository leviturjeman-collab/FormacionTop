...
export async function writeCourseShards(course, publicDir, locale) {
  ...
  const index = {
    ...
    prompts: course.prompts.map(family => ({ ...family, intro: family.intro.slice(0,240), canDo: [], cantDo: [], tips: [], prompts: family.prompts.map(prompt => ({ ...prompt, example: undefined, exampleValues: undefined, prompt: '', fill: [], expect: '', next: '', source: '', categoryId: '', when: '' })) })),
    ...
    curso: course.curso.map(item => ({ ...item, practiceLab: undefined, why: '', theory: [], projectWorkbook: undefined, tasks: item.tasks.map(task => ({ title: task.title, where: '', action: '', expect: '' })), errors: [], matters: [], ignore: [], canDo: [], cantDo: [], words: [], pieces: [] })),
    ...
    agents: course.agents.map(item => ({ ...item, practiceLab: undefined, files: [], setup: item.setup.map(step => ({ title: step.title, action: '', expect: '' })), tests: [], prompts: [] })),
  }
