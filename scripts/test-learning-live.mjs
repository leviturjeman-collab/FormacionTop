...
try {
  ...
  await page.getByRole('heading',{name:'Crear la primera función que funciona'}).waitFor({timeout:60000})
  ...
  await page.getByRole('button',{name:'Salir',exact:true}).click();await login(learner.pin)
  ...
  assert.match(await page.locator('.st-saved-resources').innerText(),/Crear la primera función/)
  await page.goto(base+'#/classes');await page.getByRole('heading',{name:'Acceso restringido'}).waitFor()
  await page.getByRole('button',{name:'Salir',exact:true}).click();await login(pin)
  await page.getByRole('heading',{name:'Clases y profesores'}).waitFor({timeout:60000})
  await page.getByRole('heading',{name:'Crear clase',exact:true}).waitFor()
  assert.deepEqual(errors,[])
  await page.getByRole('button',{name:'Salir',exact:true}).click()
  console.log('PASS production: Codex lesson, remote bookmark and completion survive sign-out/sign-in, learner class restriction, administrator class UI, no runtime errors. Temporary learner archived.')
} finally {
