...
tags: ["ai-academy", "plantilla_entregable", "transversal", "transversal"]
...
Cada lección importante debe tener mínimo 1000 palabras antes de convertirse en clase final. Si el tema es una función, herramienta o flujo crítico, debe poder ocupar unas cinco páginas de trabajo cuando se exporte a documento: explicación, modelo mental, uso paso a paso, errores, práctica, evaluación y fuentes.

No basta con decir qué es una herramienta. Hay que explicar cómo funciona, cuándo se usa, cuándo no se usa, qué entradas necesita, qué salida produce, qué riesgos tiene y cómo se verifica.
...
Para Plantilla leccion, la pregunta central es: que cambia en el trabajo del alumno cuando domina este tema. Si el tema es una fuente, debe aprender a usarla para no depender de opiniones genericas. Si es una leccion, debe aprender a explicar y aplicar el concepto. Si es un laboratorio, debe construir algo, romperlo y repararlo. Si es una evaluacion, debe demostrar competencia observable. Si es una plantilla, debe producir materiales consistentes. Si es un modulo, debe conectar varias lecciones en una progresion clara.
...
Un buen check para Plantilla leccion es pedir al alumno que lo explique en tres niveles. En 60 segundos debe dar una version simple. En modelo mental debe dibujar o describir las piezas. En profundidad profesional debe mencionar limites, riesgos, verificaciones y fuentes. Si no puede hacer los tres niveles, la leccion necesita mas trabajo antes de pasar al laboratorio.
...
Dominio: plantillas profesionales.

Este archivo trabaja principalmente sobre: formatos reutilizables para lecciones, laboratorios, proyectos y fuentes.

El resultado esperado no es solo conocimiento. El resultado esperado es: plantilla lista para producir material premium.
...
# Capa ultra-premium AAAA+ - Plantilla Leccion
...
Perfil principal al que sirve: productor de material educativo.

Artefacto premium esperado: plantilla reutilizable para clase, laboratorio, proyecto o fuente.

Prueba de dominio: otro creador puede producir material consistente.
...
Usar **Plantilla leccion** para producir el entregable definido en la metadata: **plantilla reutilizable**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Plantilla leccion**.
