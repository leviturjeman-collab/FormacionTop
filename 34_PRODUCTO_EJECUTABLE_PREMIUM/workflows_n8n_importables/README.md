...
Todos son didacticos: importar, adaptar credenciales, probar payload feliz y payload roto.

| Workflow | Uso | Archivo |
|---|---|---|
| Lead Triage | Clasificar leads por prioridad | `lead_triage.json` |
| Email Summarizer | Resumir emails largos | `email_summarizer.json` |
| Webhook Validator | Validar payloads entrantes | `webhook_validator.json` |
| CRM Updater | Actualizar CRM ficticio | `crm_updater.json` |
| Human Approval | Requerir aprobacion humana | `human_approval.json` |
| RAG Assistant | Responder con documentos | `rag_assistant.json` |
| Slack Notifier | Notificar evento | `slack_notifier.json` |
| Invoice Extractor | Extraer campos de factura | `invoice_extractor.json` |
| Support Ticket Router | Asignar tickets | `support_ticket_router.json` |
| Ecommerce Product Enricher | Enriquecer productos | `ecommerce_product_enricher.json` |
| Student Onboarding | Clasificar alumnos | `student_onboarding.json` |
| Content Repurposer | Convertir contenido | `content_repurposer.json` |
| Meeting Notes Processor | Procesar notas | `meeting_notes_processor.json` |
| API Error Monitor | Monitorizar errores | `api_error_monitor.json` |
| Multi LLM Comparator | Comparar modelos | `multi_llm_comparator.json` |
| GDPR Consent Checker | Revisar consentimiento | `gdpr_consent_checker.json` |
| Portfolio Case Builder | Crear caso portfolio | `portfolio_case_builder.json` |
| Deployment Checklist | Checklist deploy | `deployment_checklist.json` |
| Secret Leak Alert | Detectar secretos | `secret_leak_alert.json` |
| Weekly Report Generator | Informe semanal | `weekly_report_generator.json` |
| Browser Research Task | Tarea web controlada | `browser_research_task.json` |
| Voice Transcript Pipeline | Transcripcion audio | `voice_transcript_pipeline.json` |
| Vector Ingestion | Ingesta RAG | `vector_ingestion.json` |
| Cost Guardrail | Control coste | `cost_guardrail.json` |
| Client Delivery Pack | Pack entrega cliente | `client_delivery_pack.json` |
...
Este archivo debe tratarse como una pieza de **workflow de automatizacion** dentro de la formacion. Su funcion no es rellenar la boveda con contenido, sino ayudar a una persona real a producir un resultado verificable. En esta academia, una nota esta terminada cuando el alumno puede abrirla, entender el objetivo, preparar los materiales, ejecutar una practica, provocar un fallo, repararlo, guardar evidencia y defender lo aprendido con sus palabras.

El proposito concreto de esta pieza es **importar o documentar un flujo n8n con entradas, salidas, validacion y errores controlados**. Eso significa que el profesor no debe usarla como lectura pasiva. Debe convertirla en actividad, demostracion, evaluacion o entregable. Si el alumno solo lee y asiente, todavia no hay aprendizaje profesional. Si el alumno produce una evidencia, detecta un limite y explica una decision, entonces la pieza empieza a cumplir su funcion.
