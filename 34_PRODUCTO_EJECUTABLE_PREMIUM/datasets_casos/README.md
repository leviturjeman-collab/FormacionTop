# Datasets y casos ficticios

Datasets para leads, tickets, facturas, emails, ecommerce, alumnos, agencia y RAG. Cada dataset tiene caso feliz y caso roto: campos vacios, datos ambiguos o formato insuficiente.
...
Este archivo debe tratarse como una pieza de **dataset de practica** dentro de la formacion. Su funcion no es rellenar la boveda con contenido, sino ayudar a una persona real a producir un resultado verificable. En esta academia, una nota esta terminada cuando el alumno puede abrirla, entender el objetivo, preparar los materiales, ejecutar una practica, provocar un fallo, repararlo, guardar evidencia y defender lo aprendido con sus palabras.

El proposito concreto de esta pieza es **entrenar lectura de datos, limpieza, validacion, ambiguedad y criterios de privacidad**. Eso significa que el profesor no debe usarla como lectura pasiva. Debe convertirla en actividad, demostracion, evaluacion o entregable. Si el alumno solo lee y asiente, todavia no hay aprendizaje profesional. Si el alumno produce una evidencia, detecta un limite y explica una decision, entonces la pieza empieza a cumplir su funcion.
