...
tags: ["ai-academy", "glosario", "transversal", "transversal"]
...
Sistema que usa un modelo para decidir pasos y puede usar herramientas o contexto externo para lograr un objetivo.

## Workflow

Secuencia de pasos diseñada para producir una salida repetible.

## Prompt

Petición concreta enviada al modelo.

## Instruction

Regla o guía más estable que condiciona cómo debe comportarse el sistema o agente.

## Skill

Paquete reutilizable de instrucciones, ejemplos, scripts o recursos para realizar una tarea especializada.

## Tool

Función o capacidad que el modelo puede invocar para leer, calcular, consultar o actuar sobre sistemas externos.

## MCP

Protocolo para conectar modelos y agentes con herramientas, datos y sistemas externos de forma estandarizada.

## RAG

Patrón donde se recupera información relevante de una base de conocimiento y se entrega al modelo como contexto.

## HITL

Human-in-the-loop. Patrón donde una persona revisa, aprueba o decide antes de una acción sensible.

## Eval

Prueba o conjunto de pruebas para medir si un sistema de IA se comporta como se espera.

## Token

Unidad de texto u otro contenido procesada por un modelo. Sirve para medir contexto, coste y límites.
---

# Desarrollo completo - Glosario Base

## Funcion dentro de la academia

Esta nota forma parte de la boveda de Obsidian de AI Professional Academy y debe leerse como material de estudio, no como un simple indice. Su funcion es desarrollar el tema Glosario base dentro del contexto de vocabulario tecnico, diferencias conceptuales y lenguaje comun para toda la academia. El objetivo es que el alumno pueda abrir este archivo, entender que papel ocupa en la formacion, estudiar el concepto con calma, practicarlo y despues convertirlo en una idea propia, un documento, una presentacion, un laboratorio o una pieza de un proyecto final.
...
Para Glosario base, la pregunta central es: que cambia en el trabajo del alumno cuando domina este tema. Si el tema es una fuente, debe aprender a usarla para no depender de opiniones genericas. Si es una leccion, debe aprender a explicar y aplicar el concepto. Si es un laboratorio, debe construir algo, romperlo y repararlo. Si es una evaluacion, debe demostrar competencia observable. Si es una plantilla, debe producir materiales consistentes. Si es un modulo, debe conectar varias lecciones en una progresion clara.
...
Un buen check para Glosario base es pedir al alumno que lo explique en tres niveles. En 60 segundos debe dar una version simple. En modelo mental debe dibujar o describir las piezas. En profundidad profesional debe mencionar limites, riesgos, verificaciones y fuentes. Si no puede hacer los tres niveles, la leccion necesita mas trabajo antes de pasar al laboratorio.
...
# Capa AAAA+ - Glosario Base

## Proposito profesional de esta nota

Esta nota queda elevada al estandar AAAA+. Su funcion dentro de la boveda es servir como pieza profesional de glosario profesional, precision conceptual y lenguaje comun para el alumno. No debe leerse como una nota aislada ni como un resumen rapido. Debe poder ayudar a un alumno a entender el tema, aplicarlo a su trabajo, convertirlo en proyecto, evaluarlo y defenderlo con criterio.
...
# Masterclass AAAA+ - Glosario Base
...
# Capa ultra-premium AAAA+ - Glosario Base
...
Usar **Glosario base** para producir el entregable definido en la metadata: **definiciones operativas**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Glosario base**.
