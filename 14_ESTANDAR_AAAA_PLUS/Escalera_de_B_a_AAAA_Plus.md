---
titulo: "Escalera de B a AAAA Plus"
...
---
# Escalera de proyecto B a AAAA+

## Objetivo

Este documento explica como convertir un proyecto Nivel B en un proyecto AAAA+. En la boveda, Nivel B significa normalmente navegador, cuentas externas y herramientas web. Puede ser ChatGPT, Claude, n8n cloud, GitHub web, Gemini AI Studio o una herramienta conectada sin terminal local. Nivel B no es malo. De hecho, muchos proyectos muy utiles empiezan ahi. El problema aparece cuando un proyecto se queda en "lo hice en una web" y no tiene estructura, repetibilidad ni evaluacion.

AAAA+ es el proceso de endurecerlo.

## Version B

Un proyecto B suele tener:
...
- Archivos subidos.
- Resultado manual.
- Poca o ninguna automatizacion local.
- Poco codigo o nada de codigo.

Ejemplo: "Uso ChatGPT para resumir reuniones".

Esto puede ser suficiente para probar una idea, pero no para convertirla en formacion premium.

## Version A

Para subir a A, el proyecto necesita:

- Objetivo escrito.
- Entrada definida.
- Salida definida.
- Prompt o workflow guardado.
- Checklist de revision.
- Fuentes si aplica.
- Ejemplo bueno.
- Ejemplo malo.

Ejemplo: "Tengo una plantilla que convierte notas de reunion en decisiones, tareas, owners, dudas y riesgos".

## Version AA

Para subir a AA:

- Se añade una plantilla reusable.
- Se documentan variaciones.
- Se separan tareas frecuentes.
- Se indica cuando usarlo y cuando no.
- Se define evaluacion minima.
- Se añade privacidad o limites.

Ejemplo: "La plantilla no inventa responsables, marca huecos y separa hechos de supuestos".

## Version AAA

Para subir a AAA:

- Se convierte en skill, laboratorio o proyecto.
- Tiene `CHECK -> DO -> BREAK -> FIX -> EXPLAIN`.
- Tiene error provocado.
- Tiene rubrica.
- Tiene compatibilidad por sistema operativo si requiere herramientas.
- Tiene entregable de portfolio.

Ejemplo: "Skill de follow-up con caso feliz, caso ambiguo y caso roto".

## Version AAAA+

Para llegar a AAAA+:

- Tiene fuente oficial o patron real.
- Tiene version basica, intermedia y avanzada.
- Tiene artefacto real verificable.
- Tiene evaluacion por otra persona.
- Tiene documentacion de instalacion o ejecucion si aplica.
- Tiene criterios de seguridad.
- Tiene limites de uso.
- Tiene salida exportable a Obsidian, documento, presentacion o proyecto.
- Puede defenderse oralmente.

Ejemplo final: "Sistema de meeting follow-up con skill, plantilla, rubrica, casos de prueba, privacidad, ejemplos, presentacion del metodo y checklist de entrega".

## Cuándo hace falta codigo real

No hace falta codigo real para todos los proyectos B. Si el objetivo es comunicacion, documentacion, decision o formacion, puede bastar con prompts, plantillas, rubricas y ejemplos reales. Pero si el proyecto promete ejecutar acciones, conectar APIs, validar JSON, procesar archivos automaticamente, desplegar, medir coste, consultar base de datos, usar MCP o producir una app, entonces necesita artefacto tecnico real.

Codigo real puede ser:
...
## Que no vale como AAAA+

No vale:

- Un prompt suelto sin contexto.
- Una respuesta bonita sin revision.
- Una automatizacion que solo funciona una vez.
- Una skill sin descripcion clara.
- Un workflow sin errores provocados.
- Un proyecto sin fuentes.
- Un proyecto tecnico sin artefacto verificable.
- Una demo sin criterio de terminado.

## Ejemplo completo

Proyecto B: "Quiero crear una propuesta comercial con ChatGPT".

AAAA+:

1. Plantilla de discovery.
2. Prompt de propuesta.
3. Checklist de datos faltantes.
4. Ejemplo de propuesta buena.
5. Ejemplo de propuesta mala.
6. Skill `proposal-builder`.
7. Rubrica de evaluacion.
8. Version exportable a docx.
9. Presentacion de 5 slides.
10. Politica de privacidad para datos de cliente.

## Resultado

Esta escalera permite que cualquier idea pequeña se convierta en una pieza seria del curso. No se trata de complicar por complicar. Se trata de hacer que cada proyecto tenga profundidad, utilidad y defensa.

## Conversion por tipo de proyecto

### Documento

B: documento generado con IA.

AAAA+: documento con objetivo, audiencia, fuentes, estructura, revision, plantilla, version editable, version final y rubrica.

### Presentacion

B: slides generadas.

AAAA+: narrativa, audiencia, notas del presentador, fuentes, diseño consistente, version exportable y evaluacion.

### Skill

B: prompt reutilizable.

AAAA+: `SKILL.md`, descripcion clara, casos de activacion, cuando no usarla, ejemplos, caso roto y evaluacion.

### Workflow n8n

B: workflow cloud que funciona una vez.

AAAA+: export JSON, diagrama, inputs, outputs, credenciales documentadas, errores, logs, retry, HITL si aplica y checklist.

### Proyecto de codigo

B: cambio hecho con Codex o Claude.

AAAA+: repo, diff, pruebas, comandos, explicacion, rollback, code review y documentacion por sistema operativo.

### Proyecto Gemini

B: prompt en AI Studio.

AAAA+: Interactions API, schema o function calling, API key segura, request reproducible, validacion, coste estimado y fuentes.

## Preguntas de escalado

Antes de subir un proyecto, responde:

- Que valor real aporta.
- Que parte se repite.
- Que puede fallar.
- Que evidencia demuestra que funciona.
- Que parte necesita humano.
- Que herramienta sobra.
- Que herramienta falta.
- Que salida puede entrar al portfolio.

Si no hay respuesta clara, no se escala todavia. Primero se aclara el proyecto.

## Cierre

El salto de B a AAAA+ es menos tecnico de lo que parece. Es una disciplina: definir, construir, probar, romper, reparar, documentar y defender. Esa disciplina se puede aplicar a prompts, documentos, skills, codigo, agentes y sistemas.

## Aplicacion a los proyectos del vault

El asistente de trabajo pasa de B a AAAA+ cuando deja de ser un prompt y se convierte en sistema de workflows personales con plantillas, casos y rubrica. El agente Codex pasa a AAAA+ cuando tiene repo, tarea, diff, pruebas, review y defensa. El workflow n8n pasa a AAAA+ cuando tiene export, manejo de errores, credentials documentadas y logs. El sistema RAG pasa a AAAA+ cuando tiene fuentes, estrategia de recuperacion, evals y limites. El agente de produccion pasa a AAAA+ cuando incluye permisos, HITL, logs, coste, rollback y responsable humano.

Cada proyecto debe subir por escalones. Si intenta saltar directo, se vuelve fragil. Esta escalera permite saber que falta sin destruir lo que ya existe.
---

<!-- CAPA_AAAA_PLUS_APLICADA -->

# Capa AAAA+ - Escalera De B A Aaaa Plus
...
# Masterclass AAAA+ - Escalera De B A Aaaa Plus
...
# Capa ultra-premium AAAA+ - Escalera De B A Aaaa Plus
...
Usar **Escalera de B a AAAA Plus** para producir el entregable definido en la metadata: **rubrica o instrumento de evaluacion**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Escalera de B a AAAA Plus**.
