...
Esta rubrica evalua si un proyecto, laboratorio o skill alcanza nivel AAAA+. Se puede aplicar a proyectos tecnicos y no tecnicos. La pregunta no es si el proyecto parece impresionante. La pregunta es si es claro, util, reproducible, verificable, seguro y defendible.

## Criterios

| Criterio | Basico | Intermedio | Avanzado | AAAA+ |
|---|---|---|---|---|
| Objetivo | Existe pero es general | Esta definido | Tiene usuario y resultado | Tiene problema, usuario, valor y limites |
| Fuentes | No claras | Algunas referencias | Fuentes oficiales | Fuentes oficiales enlazadas y usadas |
| Entregable | Texto o idea | Plantilla/prototipo | Artefacto verificable | Artefacto reutilizable y defendible |
| Ejecucion | Manual | Repetible con guia | Reproducible | Reproducible por otra persona |
| Error provocado | No hay | Hay error simple | Hay BREAK/FIX | Hay casos feliz, ambiguo y roto |
| Evaluacion | Opinion | Checklist | Rubrica | Rubrica + evidencia |
| Seguridad | No considerada | Avisos basicos | Permisos y secretos | Minimo privilegio, HITL, logs si aplica |
| Sistema operativo | No indicado | Indicado | Instrucciones parciales | Windows/macOS/Linux o alcance declarado |
| Codigo/artefacto | No existe | Parcial | Real | Real, explicado y verificable cuando aplica |
| Portfolio | No listo | Puede mostrarse | Bien documentado | Defendible como caso profesional |

## Puntuacion

Cada criterio puede puntuarse de 1 a 4:
...
- 10-18: borrador.
- 19-27: util pero incompleto.
- 28-34: buen proyecto.
- 35-40: AAAA+.

## Requisitos minimos para AAAA+

Un proyecto no puede ser AAAA+ si falta cualquiera de estos:

- Objetivo claro.
- Entregable real.
- Fuente o justificacion.
- Evaluacion.
- Error provocado.
- Criterio de terminado.
- Instrucciones para otra persona.

Si es tecnico, ademas necesita:

- Artefacto tecnico real.
- Verificacion.
- Seguridad basica.
- Instrucciones por entorno o sistema operativo.
...
- Como se verifica.
- Que evidencia hay.
- Como se llevaria a produccion o portfolio.

## Uso de la rubrica

Esta rubrica debe usarse al final de cada fase avanzada y en todos los proyectos finales. Tambien puede usarse para mejorar proyectos B: se puntua el estado actual, se detectan huecos y se define el siguiente salto.

## Como usarla para mejorar

Primero se puntua sin piedad. Si el proyecto tiene buen objetivo pero no tiene evaluacion, no se maquilla. Si tiene codigo pero no tiene fuentes, baja. Si tiene presentacion bonita pero no tiene evidencia, baja. La rubrica sirve porque obliga a ver huecos.

Despues se eligen tres mejoras maximas. No se intenta arreglar todo a la vez. Por ejemplo:

- Añadir casos de prueba.
- Mejorar documentacion por sistema operativo.
- Crear error provocado.

Cuando esas tres mejoras estan hechas, se vuelve a puntuar.

## Ejemplo de evaluacion

Proyecto: workflow de meeting follow-up.

Objetivo: 4 si esta claro.

Fuentes: 3 si usa OpenAI Skills & Plugins.

Entregable: 3 si tiene plantilla; 4 si tiene skill.

Error provocado: 4 si incluye notas ambiguas y responsables ausentes.

Seguridad: 4 si incluye privacidad y no usar datos sensibles.

Portfolio: 4 si incluye antes/despues, plantilla y defensa.

## Decision final

Un proyecto AAAA+ no necesita puntuacion perfecta en todo, pero no puede tener huecos criticos. Si falla en seguridad, evaluacion o entregable real, no pasa. Si es fuerte en todo y otra persona puede usarlo, defenderlo y aprender de el, entonces si merece el sello AAAA+.

## Aplicacion practica

La rubrica debe usarse con evidencias, no con sensaciones. Si el alumno dice que el proyecto es reproducible, debe mostrar instrucciones. Si dice que es seguro, debe mostrar permisos o limites. Si dice que esta evaluado, debe mostrar casos. Si dice que sirve para portfolio, debe mostrar una pieza presentable.

Para proyectos no tecnicos, la evidencia puede ser documento, presentacion, plantilla, skill o rubrica. Para proyectos tecnicos, la evidencia puede ser codigo, workflow, JSON, tests, logs, diff, API request o export. La rubrica acepta ambos caminos, pero no acepta ausencia de evidencia.

Usarla bien convierte la evaluacion en mejora continua. Cada puntuacion baja se convierte en una tarea concreta de mejora.

## Uso en clase

La rubrica puede usarse en tres momentos. Al principio, sirve para diagnosticar el estado del proyecto. A mitad, sirve para decidir que mejorar. Al final, sirve para defensa. No debe aparecer solo al final como examen sorpresa.

El alumno debe aprender a autoevaluarse. Primero puntua su proyecto. Despues otra persona lo revisa. Luego compara diferencias. Si el alumno se puso un 4 en reproducibilidad pero nadie mas puede ejecutarlo, aprende que la evidencia importa mas que la intencion.

## Preguntas de revision

- Que evidencia tienes.
- Que fuente oficial lo respalda.
- Que parte repetiria otra persona.
- Que pasa si falla.
- Que falta para subir un punto.
- Que herramienta sobra.
- Que riesgo no has controlado.

Estas preguntas convierten la rubrica en conversacion profesional.

## Cierre

La rubrica no castiga. Orienta. Su trabajo es mostrar el siguiente paso concreto para que un proyecto suba de nivel sin perder claridad.

Un proyecto AAAA+ no nace perfecto. Se construye por iteraciones, con evidencia y con revision honesta.

Cuando el alumno entiende esto, deja de temer la evaluacion y empieza a usarla como herramienta de diseño.

El profesor tambien gana claridad: puede señalar exactamente que falta y evitar correcciones vagas. La rubrica convierte feedback en accion concreta.

Una buena revision termina con tres proximos pasos, no con una opinion general.

La rubrica tambien debe guardarse junto al proyecto, no separada. Asi, cuando el alumno revise su portfolio semanas despues, entendera por que tomo ciertas decisiones y que mejoras quedaron pendientes. Esa trazabilidad convierte cada entrega en aprendizaje acumulado.
---

<!-- CAPA_AAAA_PLUS_APLICADA -->

# Capa AAAA+ - Rubrica Aaaa Plus
...
# Masterclass AAAA+ - Rubrica Aaaa Plus
...
# Capa ultra-premium AAAA+ - Rubrica Aaaa Plus
...
Usar **Rubrica AAAA Plus** para producir el entregable definido en la metadata: **rubrica o instrumento de evaluacion**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Rubrica AAAA Plus**.
