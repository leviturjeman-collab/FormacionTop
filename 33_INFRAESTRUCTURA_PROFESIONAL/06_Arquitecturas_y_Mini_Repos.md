---
titulo: "Arquitecturas y mini repos clonables"
tipo: "guia_arquitectura"
...
tags: ["ai-academy", "arquitectura", "portfolio", "repos"]
entregable: "diagramas y repos base"
---
# Arquitecturas y mini repos clonables

## SaaS IA basico

```mermaid
flowchart LR
  U[Usuario] --> FE[Frontend Vercel]
  FE --> API[API]
  API --> DB[(Postgres)]
  API --> LLM[LLM Provider]
  API --> LOGS[Logs/Sentry]
```
...
```mermaid
flowchart LR
  TASK[Tarea] --> AG[Agente]
  AG --> DEC{Decision}
  DEC --> TOOL[Tool segura]
  DEC --> HUMAN[Aprobacion humana]
  TOOL --> LOG[Audit log]
```

## Mini repos clonables

| Repo | Stack | Entregable |
|---|---|---|
| `starter-next-vercel-env` | Next.js + Vercel | deploy con env vars |
| `starter-n8n-lead-triage` | n8n + webhook | workflow exportado |
| `starter-rag-postgres` | Postgres + pgvector | RAG minimo |
| `starter-multi-llm-router` | LiteLLM | comparador de modelos |
| `starter-video-remotion` | Remotion + FFmpeg | video programatico |
| `starter-playwright-e2e` | Playwright | test E2E |
| `starter-supabase-auth` | Supabase | login + tabla |
| `starter-mcp-permissions` | MCP | mapa de tools |
| `starter-observability` | Sentry + PostHog | evento + error test |
| `starter-docker-stack` | Docker Compose | Postgres + app |
...
Diagrama, repo base, variables, test, caso roto y defensa.

<!-- DESARROLLO_EXTENSO_PREMIUM_2026_08_18 -->

# Desarrollo extenso premium - 06 Arquitecturas Y Mini Repos
