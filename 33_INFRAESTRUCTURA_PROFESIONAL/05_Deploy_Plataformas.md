---
titulo: "Deploy por plataforma"
tipo: "guia_deploy"
nivel: "intermedio_avanzado"
fase: "produccion"
estado: "investigacion_actualizada"
ultima_revision: "2026-08-18"
fuentes: ["https://vercel.com/docs/cli", "https://developers.cloudflare.com/workers/wrangler/", "https://docs.railway.com/cli", "https://render.com/docs/cli", "https://fly.io/docs/flyctl/install/", "https://supabase.com/docs"]
tags: ["ai-academy", "deploy", "vercel", "cloudflare", "railway", "render", "fly"]
entregable: "matriz de deploy"
---
# Deploy por plataforma

| Plataforma | Mejor para | CLI/comando base | Riesgo |
|---|---|---|---|
| Vercel | Frontends, Next.js, previews | `npm i -g vercel` | env vars mal configuradas |
| Cloudflare Workers | Edge/serverless | `npm create cloudflare@latest` o `npx wrangler` | compatibilidad Node APIs |
| Railway | Apps y servicios rapidos | `bash <(curl -fsSL railway.com/install.sh) -y` | costes por servicios activos |
| Render | Web services y blueprints | Render CLI/dashboard | build/start mal definidos |
| Fly.io | Apps globales y maquinas | `brew install flyctl` | networking, volumes, regiones |
| Supabase | Backend Postgres/Auth | `npm install -g supabase` | service role en cliente |
| VPS Docker | Control total | `docker compose up -d` | mantenimiento y seguridad |

## Flujo profesional

1. Probar local.
2. Crear `.env.example`.
3. Configurar variables en preview.
4. Desplegar preview.
5. Probar caso feliz y roto.
6. Revisar logs.
...
# Desarrollo extenso premium - 05 Deploy Plataformas
