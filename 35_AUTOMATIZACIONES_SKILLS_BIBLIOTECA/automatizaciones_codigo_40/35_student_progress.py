...
Uso: python 35_student_progress.py --demo
     python 35_student_progress.py --input entrada.json
     echo JSON | python 35_student_progress.py
...
def _process(data):
    required_steps=data.get('required'); complete=data.get('completed',[])
    if not isinstance(required_steps,list) or not required_steps or not isinstance(complete,list): raise ValueError('required no vacío y completed deben ser listas')
    required_set=set(map(str,required_steps)); done=set(map(str,complete)) & required_set
    return {'completed':sorted(done),'missing':sorted(required_set-done),'percent':round(len(done)/len(required_set)*100,1),'ready':done==required_set}
...
DEMO={'required': ['brief', 'demo', 'tests'], 'completed': ['brief', 'demo', 'demo']}
