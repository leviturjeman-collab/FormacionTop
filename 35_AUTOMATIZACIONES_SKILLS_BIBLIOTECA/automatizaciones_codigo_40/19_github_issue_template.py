...
Uso: python 19_github_issue_template.py --demo
     python 19_github_issue_template.py --input entrada.json
     echo JSON | python 19_github_issue_template.py
...
def _process(data):
    title=required(data,'title'); problem=required(data,'problem'); expected=required(data,'expected')
    steps=data.get('steps',[])
    if not isinstance(steps,list): raise ValueError('steps debe ser lista')
    return {'title':title,'body':'## Problema\n'+problem+'\n\n## Reproducción\n'+'\n'.join(f'{i+1}. {s}' for i,s in enumerate(steps))+'\n\n## Resultado esperado\n'+expected+'\n\n## Evidencia\nAdjuntar logs anonimizados y versión.'}
...
DEMO={'title': 'Guardar pedido', 'problem': 'Se pierde al recargar', 'expected': 'Pedido persistido', 'steps': ['Crear pedido', 'Recargar']}
