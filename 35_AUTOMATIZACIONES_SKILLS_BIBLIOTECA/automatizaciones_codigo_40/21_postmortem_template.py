...
Uso: python 21_postmortem_template.py --demo
     python 21_postmortem_template.py --input entrada.json
     echo JSON | python 21_postmortem_template.py
...
def _process(data):
    incident=required(data,'incident'); impact=required(data,'impact')
    sections={'Impacto':impact,'Cronología':'\n'.join('- '+str(v) for v in data.get('timeline',[])),'Causa confirmada':str(data.get('cause','Pendiente de investigar')),'Acciones':'\n'.join('- [ ] '+str(v) for v in data.get('actions',[]))}
    return {'markdown':'# Postmortem: '+incident+'\n\n'+'\n\n'.join('## '+k+'\n'+v for k,v in sections.items()),'review_required':True}
...
DEMO={'incident': 'Servicio no disponible', 'impact': '5 pedidos retrasados', 'timeline': ['10:00 alarma', '10:12 recuperado'], 'actions': ['Probar rollback']}
