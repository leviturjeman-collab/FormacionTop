...
Uso: python 20_release_notes.py --demo
     python 20_release_notes.py --input entrada.json
     echo JSON | python 20_release_notes.py
...
def _process(data):
    version=required(data,'version'); commits=data.get('commits')
    if not isinstance(commits,list): raise ValueError('commits debe ser lista')
    groups={'Novedades':[],'Correcciones':[],'Cambios internos':[]}
    for c in commits:
        text=str(c); group='Novedades' if text.startswith('feat') else 'Correcciones' if text.startswith('fix') else 'Cambios internos'; groups[group].append(text.split(':',1)[-1].strip())
    return {'markdown':'# '+version+'\n\n'+'\n\n'.join('## '+k+'\n'+'\n'.join('- '+v for v in values) for k,values in groups.items() if values),'commits_processed':len(commits)}
...
DEMO={'version': '1.0', 'commits': ['feat: exportar pedidos', 'fix: conservar sesión', 'chore: dependencias']}
