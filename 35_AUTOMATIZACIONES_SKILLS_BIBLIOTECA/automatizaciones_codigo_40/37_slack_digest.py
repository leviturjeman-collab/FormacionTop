...
Uso: python 37_slack_digest.py --demo
     python 37_slack_digest.py --input entrada.json
     echo JSON | python 37_slack_digest.py
...
def _process(data):
    messages=records(data,'messages'); groups={}
    for m in messages: groups.setdefault(str(m.get('user','unknown')),[]).append(required(m,'text'))
    return {'text':'\n\n'.join(user+':\n'+'\n'.join('- '+t for t in texts) for user,texts in groups.items()),'message_count':len(messages),'users':len(groups)}
...
DEMO={'messages': [{'user': 'Ana', 'text': 'Terminado pedido 21'}, {'user': 'Luis', 'text': 'Bloqueo: falta acceso'}]}
