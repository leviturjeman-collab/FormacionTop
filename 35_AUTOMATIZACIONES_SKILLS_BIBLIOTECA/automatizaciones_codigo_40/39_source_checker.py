...
Uso: python 39_source_checker.py --demo
     python 39_source_checker.py --input entrada.json
     echo JSON | python 39_source_checker.py
...
def _process(data):
    url=required(data,'url'); text=required(data,'text'); claim=required(data,'claim'); flags=[]
    parsed=urllib.parse.urlparse(url)
    if parsed.scheme!='https': flags.append('non_https')
    if not data.get('author'): flags.append('missing_author')
    try: date.fromisoformat(str(data.get('date','')))
    except ValueError: flags.append('missing_or_invalid_date')
    return {'domain':parsed.hostname,'flags':flags,'claim_literal_in_text':claim.casefold() in text.casefold(),'verdict':'requires_human_verification','scope':'metadata checklist, not factual truth or domain reputation'}
...
DEMO={'url': 'https://example.com/report', 'author': 'Equipo', 'date': '2026-09-01', 'claim': 'Ventas suben', 'text': 'Informe: ventas suben'}
