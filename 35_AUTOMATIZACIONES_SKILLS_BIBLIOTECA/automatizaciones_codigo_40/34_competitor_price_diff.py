...
Uso: python 34_competitor_price_diff.py --demo
     python 34_competitor_price_diff.py --input entrada.json
     echo JSON | python 34_competitor_price_diff.py
...
def _process(data):
    previous=money(required(data,'previous')); current=money(required(data,'current'))
    return {'previous':previous,'current':current,'difference':round(current-previous,2),'changed':abs(current-previous)>=0.005,'percent':round((current-previous)/previous*100,2) if previous else None,'currency':str(data.get('currency','EUR'))}
...
DEMO={'previous': '1.234,56', 'current': '1.234,56', 'currency': 'EUR'}
