...
Uso: python 27_qdrant_health.py --demo
     python 27_qdrant_health.py --input entrada.json
     echo JSON | python 27_qdrant_health.py
...
def _process(data):
    ...
    if response is None:
        base=required(data,'url').rstrip('/')
        if base not in ['http://127.0.0.1:6333','http://localhost:6333']: raise ValueError('Esta demo solo permite Qdrant local')
        req=urllib.request.Request(base+'/collections',headers={'api-key':os.environ.get('QDRANT_API_KEY','')})
        with urllib.request.urlopen(req,timeout=5) as r: response=json.load(r)
    if not isinstance(response,dict): raise ValueError('Respuesta inválida')
    return {'healthy':response.get('status')=='ok','collections':response.get('result',{}).get('collections',[])}
...
DEMO={'response': {'status': 'ok', 'result': {'collections': []}}}
