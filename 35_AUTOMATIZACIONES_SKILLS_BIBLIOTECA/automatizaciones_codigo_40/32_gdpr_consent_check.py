...
Uso: python 32_gdpr_consent_check.py --demo
     python 32_gdpr_consent_check.py --input entrada.json
     echo JSON | python 32_gdpr_consent_check.py
...
def _process(data):
    purpose=required(data,'purpose'); email=required(data,'email')
    allowed=data.get('consent') is True
    return {'allowed':allowed,'purpose':purpose,'email':email,'reason':'explicit_consent' if allowed else 'no_explicit_consent','scope':'technical consent gate; not a legal compliance certification'}
...
DEMO={'consent': True, 'purpose': 'seguimiento solicitado', 'email': 'ana@example.com'}
