---
titulo: "CHECK DO BREAK FIX EXPLAIN"
...
tags: ["ai-academy", "guia_pedagogica", "transversal", "aprendizaje"]
...
Antes de enseñar, comprobar qué sabe el alumno. El check no es un examen largo: es una pregunta, una predicción o una microtarea.

Ejemplos:

- ¿Dónde viajará este dato en el workflow?
- ¿Qué permiso necesita este agente?
- ¿Qué pasará si la API key no existe?
- ¿Dónde mirarías el coste de esta ejecución?

## DO

El alumno construye la versión correcta con ayuda.

La práctica debe tener:
...
El profesor rompe el sistema de forma deliberada.
...
- 401, 403 o 429.
- JSON roto.
- Scope OAuth insuficiente.
- Expression de n8n incorrecta.
- Webhook duplicado.
- Merge conflict.
- Tool call inválida.
- Agente sin permisos.
- Agente con demasiados permisos.
- Loop de agente consumiendo tokens.

## FIX

El alumno diagnostica y repara.

Debe aprender a mirar:
...
El alumno explica qué ocurrió, cómo lo encontró, cómo lo arregló y cómo evitaría que vuelva a pasar.

Una buena explicación incluye:
...
Para CHECK DO BREAK FIX EXPLAIN, la pregunta central es: que cambia en el trabajo del alumno cuando domina este tema. Si el tema es una fuente, debe aprender a usarla para no depender de opiniones genericas. Si es una leccion, debe aprender a explicar y aplicar el concepto. Si es un laboratorio, debe construir algo, romperlo y repararlo. Si es una evaluacion, debe demostrar competencia observable. Si es una plantilla, debe producir materiales consistentes. Si es un modulo, debe conectar varias lecciones en una progresion clara.
...
Un buen check para CHECK DO BREAK FIX EXPLAIN es pedir al alumno que lo explique en tres niveles. En 60 segundos debe dar una version simple. En modelo mental debe dibujar o describir las piezas. En profundidad profesional debe mencionar limites, riesgos, verificaciones y fuentes. Si no puede hacer los tres niveles, la leccion necesita mas trabajo antes de pasar al laboratorio.
...
# Capa AAAA+ - Check Do Break Fix Explain

## Proposito profesional de esta nota

Esta nota queda elevada al estandar AAAA+. Su funcion dentro de la boveda es servir como pieza profesional de metodo pedagogico, diseno de aprendizaje, practica deliberada y evaluacion por evidencia. No debe leerse como una nota aislada ni como un resumen rapido. Debe poder ayudar a un alumno a entender el tema, aplicarlo a su trabajo, convertirlo en proyecto, evaluarlo y defenderlo con criterio.
...
# Masterclass AAAA+ - Check Do Break Fix Explain
...
# Capa ultra-premium AAAA+ - Check Do Break Fix Explain
...
Usar **CHECK DO BREAK FIX EXPLAIN** para producir el entregable definido en la metadata: **guia de clase o ruta de aprendizaje**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **CHECK DO BREAK FIX EXPLAIN**.
