---
titulo: "Prompts de investigacion"
...
Los prompts de investigacion sirven para recopilar, organizar y evaluar informacion. Deben usarse con fuentes, fechas y criterio. No se aceptan afirmaciones importantes sin origen.

## Prompt base

```text
Quiero investigar [tema] para [objetivo]. Usa solo fuentes oficiales o primarias cuando sea posible. Devuelveme:
1. resumen ejecutivo,
2. conceptos clave,
3. tabla de fuentes con enlace, fecha y autoridad,
4. contradicciones o dudas,
5. implicaciones practicas,
6. ideas para clase o proyecto.
No inventes datos. Si algo no esta claro, dilo.
```

## Prompt de comparativa

```text
Compara [herramienta A], [herramienta B] y [herramienta C] para [caso de uso]. Evalua: coste, facilidad, privacidad, integraciones, curva de aprendizaje, riesgos, mantenimiento y nivel recomendado. Termina con una recomendacion por perfil: principiante, intermedio y avanzado.
```

## Prompt de fuente critica

```text
Analiza esta fuente: [pegar texto o enlace]. Separa hechos, claims, supuestos y recomendaciones. Indica que partes requieren verificacion externa y que partes se pueden usar en una formacion.
```

## Practica

El alumno debe investigar una herramienta oficial y crear una nota de Obsidian. BREAK: usar una fuente comercial no oficial como si fuera documentacion. FIX: marcarla como secundaria y buscar fuente primaria.


---

# Capa de desarrollo AAAA+ aplicada a Prompts de investigacion

## Para que sirve este archivo dentro de la formacion

Este archivo no debe leerse como una nota aislada. Su funcion dentro de la academia es convertir el tema Prompts de investigacion en una pieza util para aprender, aplicar, construir, documentar y defender trabajo real. La pregunta que debe guiar su uso es sencilla: que puede hacer el alumno despues de leerlo que antes no podia hacer con claridad. Si la respuesta no se puede convertir en una accion observable, una practica, una plantilla, una decision de herramientas o un entregable, el archivo debe ampliarse o conectarse con otro material de la boveda.
...
Una clase basada en Prompts de investigacion no deberia terminar con "lo hemos visto". Debe terminar con algo que el alumno pueda guardar en Obsidian: una checklist, una plantilla, un prompt, un workflow, un mini informe, una rubrica, un mapa de decision o una pieza de portfolio. Ese artefacto es la prueba de aprendizaje.

## Practica recomendada

La practica recomendada consiste en tomar un problema pequeÃƒÂ±o y usar Prompts de investigacion para resolverlo de forma limitada. Primero se define el objetivo. Despues se escribe el contexto. Luego se decide que herramienta o metodo se usara. A continuacion se construye una primera version. Despues se provoca un fallo. Finalmente se documenta la reparacion.
...
Para convertir Prompts de investigacion en entregable, aÃƒÂ±ade portada, objetivo, contexto, pasos, resultado, limites, riesgos, checklist y siguiente accion. Si es para cliente, aÃƒÂ±ade alcance y mantenimiento. Si es para portfolio, aÃƒÂ±ade demo y caso de estudio. Si es para clase, aÃƒÂ±ade practica y rubrica. Si es para uso interno, aÃƒÂ±ade responsable y frecuencia de revision.
...
Usar **Prompts de investigacion** para producir el entregable definido en la metadata: **plantilla reutilizable**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Prompts de investigacion**.
