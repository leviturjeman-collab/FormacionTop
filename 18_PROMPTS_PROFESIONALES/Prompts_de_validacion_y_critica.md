...
Estos prompts sirven para mejorar calidad. Un sistema AAAA+ no termina cuando genera algo; termina cuando se revisa.
...
```text
Imagina que este proyecto falla en produccion. Dame 10 causas probables, seÃ±ales tempranas, forma de detectarlas, reparacion y prevencion.
```
...
Una clase basada en Prompts de validacion y critica no deberia terminar con "lo hemos visto". Debe terminar con algo que el alumno pueda guardar en Obsidian: una checklist, una plantilla, un prompt, un workflow, un mini informe, una rubrica, un mapa de decision o una pieza de portfolio. Ese artefacto es la prueba de aprendizaje.

## Practica recomendada

La practica recomendada consiste en tomar un problema pequeÃƒÂ±o y usar Prompts de validacion y critica para resolverlo de forma limitada. Primero se define el objetivo. Despues se escribe el contexto. Luego se decide que herramienta o metodo se usara. A continuacion se construye una primera version. Despues se provoca un fallo. Finalmente se documenta la reparacion.
...
Para convertir Prompts de validacion y critica en entregable, aÃƒÂ±ade portada, objetivo, contexto, pasos, resultado, limites, riesgos, checklist y siguiente accion. Si es para cliente, aÃƒÂ±ade alcance y mantenimiento. Si es para portfolio, aÃƒÂ±ade demo y caso de estudio. Si es para clase, aÃƒÂ±ade practica y rubrica. Si es para uso interno, aÃƒÂ±ade responsable y frecuencia de revision.
...
Usar **Prompts de validacion y critica** para producir el entregable definido en la metadata: **plantilla reutilizable**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Prompts de validacion y critica**.
