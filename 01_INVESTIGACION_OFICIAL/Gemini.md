...
Gemini debe servir para enseñar APIs modernas de IA con outputs estructurados, tools, grounding, tokens, usage, Live API y agentes administrados. La parte de tokens es especialmente útil para convertir un concepto abstracto en una práctica profesional medible.

## Lecciones que salen de este research

- Interactions API como interfaz recomendada para proyectos nuevos.
- Structured outputs.
- Function calling.
- Built-in tools.
...
- Context caching.
- Live API.
- Conteo de tokens antes de ejecutar.
- Lectura de usage después de ejecutar.
- Coste por interacción.
- Seguridad de agentes: sandboxing, mínimo privilegio, credenciales limitadas y revisión humana.

## Laboratorio clave

El alumno debe hacer el ciclo completo:

`prompt -> count tokens -> request -> response -> inspect usage -> calcular coste -> guardar usage -> visualizarlo`
...
Para Gemini, la pregunta central es: que cambia en el trabajo del alumno cuando domina este tema. Si el tema es una fuente, debe aprender a usarla para no depender de opiniones genericas. Si es una leccion, debe aprender a explicar y aplicar el concepto. Si es un laboratorio, debe construir algo, romperlo y repararlo. Si es una evaluacion, debe demostrar competencia observable. Si es una plantilla, debe producir materiales consistentes. Si es un modulo, debe conectar varias lecciones en una progresion clara.
...
Un buen check para Gemini es pedir al alumno que lo explique en tres niveles. En 60 segundos debe dar una version simple. En modelo mental debe dibujar o describir las piezas. En profundidad profesional debe mencionar limites, riesgos, verificaciones y fuentes. Si no puede hacer los tres niveles, la leccion necesita mas trabajo antes de pasar al laboratorio.
...
# Capa ultra-premium AAAA+ - Gemini
...
Usar **Gemini** para producir el entregable definido en la metadata: **manual con fuentes y practica**. El archivo debe ayudar a entender, aplicar, verificar y explicar el tema sin depender de memoria externa.
...
- Si se detecta informacion generica, convertirla en ejemplo especifico de **Gemini**.
