...
import type { ProjectLogEntry, ProjectProfile, StudentCatalog, StudentModule, StudentResource, WalkthroughProgress } from './student-types'
...
function StudentApp() {
  ...
  const [setupOpen, setSetupOpen] = useState(() => !localStorage.getItem('academy-project-v3'))
  ...
  return (
    <div className="student-app">
      <StudentSidebar view={view} open={menuOpen} progress={lessonIds.length ? completedLessons / lessonIds.length : 0} onNavigate={navigate} onClose={() => setMenuOpen(false)} />
      <main className="student-main">
        <StudentHeader project={project} view={view} onMenu={() => setMenuOpen(true)} onEditProject={() => setSetupOpen(true)} />
        {view === 'home' && <StudentHome catalog={catalog} project={project} nextLesson={nextLesson} completedLessons={completedLessons} totalLessons={lessonIds.length} walkthroughs={walkthroughs} resourcesById={resourcesById} onOpenLesson={openLesson} onNavigate={navigate} />}
        {view === 'program' && <Program catalog={catalog} resourcesById={resourcesById} walkthroughs={walkthroughs} onOpenLesson={openLesson} />}
        ...
        {view === 'resources' && <ResourceLibrary catalog={catalog} onOpenLesson={openLesson} walkthroughs={walkthroughs} />}
        {view === 'lesson' && selectedLesson && <Walkthrough resource={selectedLesson} project={project} progress={walkthroughs[selectedLesson.id] || { completedSteps: [], evidence: {} }} onBack={() => navigate('program')} onSave={(progress) => saveWalkthrough(selectedLesson.id, progress)} onLog={addProjectLog} />}
      </main>
      {setupOpen && <ProjectSetup initial={project || defaultProject} onSave={saveProject} onClose={project ? () => setSetupOpen(false) : undefined} />}
    </div>
  )
}

function StudentSidebar({ view, open, progress, onNavigate, onClose }: { view: View; open: boolean; progress: number; onNavigate: (view: View) => void; onClose: () => void }) {
  return <>
...
function StudentHeader({ project, view, onMenu, onEditProject }: { project: ProjectProfile | null; view: View; onMenu: () => void; onEditProject: () => void }) {
  ...
  return <header className="st-header"><button className="st-menu" onClick={onMenu} aria-label="Abrir menú"><Menu size={20} /></button><div><span>AI Professional Academy</span><ChevronRight size={13} /><strong>{label}</strong></div><button className="st-project-switch" onClick={onEditProject}><span>{project?.projectName || 'Configurar proyecto'}</span><Settings2 size={15} /></button></header>
}

function StudentHome({ catalog, project, nextLesson, completedLessons, totalLessons, walkthroughs, resourcesById, onOpenLesson, onNavigate }: {
  catalog: StudentCatalog; project: ProjectProfile | null; nextLesson?: StudentResource; completedLessons: number; totalLessons: number; walkthroughs: Record<string, WalkthroughProgress>; resourcesById: Map<string, StudentResource>; onOpenLesson: (resource: StudentResource) => void; onNavigate: (view: View) => void
}) {
  ...
  return <div className="st-page st-home">
    <section className="st-welcome"><div><span className="st-kicker">ESPACIO DE TRABAJO</span><h1>{project ? `Hola, ${project.studentName}.` : 'Empieza con un proyecto real.'}</h1><p>{project ? <>Estás construyendo <strong>{project.projectName}</strong> para {project.audience || 'un usuario concreto'}. La formación se aplica directamente sobre ese proyecto.</> : 'Define qué quieres construir y cada lección te indicará exactamente qué hacer, dónde hacerlo y qué evidencia guardar.'}</p></div><div className="st-overall"><span>{completedLessons}/{totalLessons}</span><small>lecciones terminadas</small></div></section>
    <div className="st-home-grid">
      <section className="st-next">
        ...
        {nextLesson && <button className="st-next-card" onClick={() => onOpenLesson(nextLesson)}><div className="st-next-type"><Play size={18} /><span>{nextLesson.kind}</span></div><div><h3>{nextLesson.title}</h3><p>{nextLesson.summary}</p><div className="st-meta"><span><Clock3 size={14} /> {nextLesson.duration} min</span><span><ClipboardCheck size={14} /> {nextLesson.walkthrough.length} pasos</span><span><FileCheck2 size={14} /> {nextLesson.deliverable}</span></div></div><ArrowRight size={20} /></button>}
      </section>
      <aside className="st-project-brief"><span className="st-kicker">TU PROYECTO</span>{project ? <><h2>{project.projectName}</h2><dl><div><dt>Problema</dt><dd>{project.problem || 'Pendiente de concretar'}</dd></div><div><dt>Usuario</dt><dd>{project.audience || 'Pendiente de concretar'}</dd></div><div><dt>Resultado</dt><dd>{project.outcome || 'Pendiente de concretar'}</dd></div></dl><button onClick={() => onNavigate('project')}>Abrir ficha del proyecto <ArrowRight size={15} /></button></> : <><h2>Sin configurar</h2><p>Crea la ficha antes de empezar para que los walkthroughs puedan aplicar cada paso.</p><button onClick={() => onNavigate('project')}>Configurar proyecto</button></>}</aside>
    </div>
    <section className="st-modules-preview"><div className="st-section-head"><div><span className="st-kicker">PROGRAMA CURADO</span><h2>Ocho etapas, un solo proyecto</h2></div><button onClick={() => onNavigate('program')}>Ver programa completo <ArrowRight size={15} /></button></div><div>{catalog.modules.map((module) => { const done = module.lessonIds.filter((id) => { const r = resourcesById.get(id); return r && (walkthroughs[id]?.completedSteps.length || 0) >= r.walkthrough.length }).length; return <article key={module.id}><span>{module.number}</span><div><strong>{module.title}</strong><small>{module.milestone}</small></div><b>{done}/{module.lessonIds.length}</b></article> })}</div></section>
  </div>
}

function Program({ catalog, resourcesById, walkthroughs, onOpenLesson }: { catalog: StudentCatalog; resourcesById: Map<string, StudentResource>; walkthroughs: Record<string, WalkthroughProgress>; onOpenLesson: (resource: StudentResource) => void }) {
  const [openModule, setOpenModule] = useState(catalog.modules[0]?.id || '')
  return <div className="st-page"><PageTitle eyebrow="PROGRAMA" title="Aprender construyendo" text="Las lecciones no reproducen la bóveda. Cada una termina en un cambio, una prueba o una evidencia dentro de tu proyecto." />
    <div className="st-program">{catalog.modules.map((module) => { const lessons = module.lessonIds.map((id) => resourcesById.get(id)).filter(Boolean) as StudentResource[]; const done = lessons.filter((lesson) => (walkthroughs[lesson.id]?.completedSteps.length || 0) >= lesson.walkthrough.length).length; const open = openModule === module.id; return <section className={`st-module ${open ? 'open' : ''}`} key={module.id}><button className="st-module-head" onClick={() => setOpenModule(open ? '' : module.id)}><span>{module.number}</span><div><h2>{module.title}</h2><p>{module.description}</p></div><div className="st-module-status"><strong>{done}/{lessons.length}</strong><small>{module.milestone}</small></div><ChevronDown size={18} /></button>{open && <div className="st-lesson-list">{lessons.map((lesson, index) => { const stepDone = walkthroughs[lesson.id]?.completedSteps.length || 0; const complete = stepDone >= lesson.walkthrough.length; return <button key={lesson.id} onClick={() => onOpenLesson(lesson)}><span className={complete ? 'complete' : ''}>{complete ? <Check size={14} /> : String(index + 1).padStart(2, '0')}</span><div><small>{lesson.kind}</small><strong>{lesson.title}</strong><p>{lesson.summary}</p></div><div><span><Clock3 size={13} /> {lesson.duration} min</span><span>{stepDone}/{lesson.walkthrough.length} pasos</span></div><ChevronRight size={17} /></button>})}</div>}</section> })}</div>
  </div>
}

function Walkthrough({ resource, project, progress, onBack, onSave, onLog }: { resource: StudentResource; project: ProjectProfile | null; progress: WalkthroughProgress; onBack: () => void; onSave: (progress: WalkthroughProgress) => void; onLog: (entry: ProjectLogEntry) => void }) {
  ...
  const projectContext = project ? `En “${project.projectName}”, que busca ${cleanOutcome} para ${project.audience || 'su usuario'},` : 'En tu proyecto,'
  ...
  return <div className="st-walkthrough">
    <header className="st-walkthrough-head"><button onClick={onBack}><ArrowLeft size={17} /> Programa</button><div><span>{resource.kind}</span><strong>{resource.title}</strong></div><div><b>{percent}%</b><i><span style={{width: `${percent}%`}} /></i></div></header>
    <div className="st-walkthrough-layout">
      ...
      <article className="st-step-workspace">
        ...
        <h1>{step.title}</h1>
        <section className="st-apply-context"><span>APLICADO A TU PROYECTO</span><p>{projectContext} {resource.projectApplication.toLowerCase()}</p></section>
        ...
        <section className="st-evidence"><div><span>4</span><div><strong>{step.evidenceLabel}</strong><small>Este texto se añadirá a la ficha de tu proyecto.</small></div></div><textarea value={evidence} onChange={(event) => setEvidence(event.target.value)} placeholder="Escribe o pega aquí el resultado concreto que has obtenido..." rows={5} /><div className="st-evidence-actions"><button onClick={() => setCurrentIndex(Math.max(0, currentIndex - 1))} disabled={currentIndex === 0}>Anterior</button><span>{evidence.trim().length < 3 ? 'Añade una evidencia para continuar' : 'Evidencia lista'}</span><button className="primary" onClick={completeStep} disabled={evidence.trim().length < 3}>{isDone ? 'Guardar cambios' : 'Completar y continuar'} <ArrowRight size={15} /></button></div></section>
      </article>
      <aside className="st-walkthrough-aside"><span className="st-kicker">RESULTADO DE LA LECCIÓN</span><p>{resource.studentOutcome}</p><dl><div><dt>Entregable</dt><dd>{resource.deliverable}</dd></div><div><dt>Duración</dt><dd>{resource.duration} minutos</dd></div><div><dt>Fuente interna</dt><dd>{resource.sourcePath.split('/').at(-1)?.replace('.md','').replaceAll('_',' ')}</dd></div></dl><div className="st-quality"><strong>Antes de terminar</strong>{resource.checks.map((check) => <span key={check}><Check size={12} />{check}</span>)}</div></aside>
    </div>
  </div>
}
...
function NoProject({onSetup}:{onSetup:()=>void}) { return <div className="st-page st-no-project"><FolderKanban size={36}/><h1>Tu proyecto todavía no tiene ficha.</h1><p>Define problema, usuario y resultado para que los walkthroughs sepan dónde aplicar cada paso.</p><button onClick={onSetup}>Configurar proyecto</button></div> }
