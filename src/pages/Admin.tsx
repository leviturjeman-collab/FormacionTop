...
export default function Admin() {
  ...
  async function save() {
    await act(async () => {
      if (editing) {
        ...
        await adminRpc('academy_admin_create', { learner: draft, initial_secret: secret })
  ...
  return <div className="st-page">
    ...
    {issued && <section className="st-panel" aria-label={tr('Credencial recién creada', 'New credential')}><h2>{tr('Entrega de acceso', 'Deliver access')}</h2><p>{tr('Identificador', 'Identifier')}: <strong>{issued.login}</strong></p><p>{tr('Esta clave solo se muestra ahora. No se incluye en fichas ni exportaciones.', 'This credential is only shown now. Profiles and exports never include it.')}</p><input aria-label={tr('Clave inicial', 'Initial credential')} readOnly value={issued.secret} onFocus={(event) => event.currentTarget.select()} /><div className="st-actions"><button className="st-btn-ghost" onClick={() => void act(async () => { await navigator.clipboard.writeText(`${tr('Identificador', 'Identifier')}: ${issued.login}\n${tr('Clave', 'Credential')}: ${issued.secret}`); setMessage(tr('Credencial copiada. Entrégala solo a su destinatario.', 'Credential copied. Deliver it only to its recipient.')) })}>{tr('Copiar acceso', 'Copy access')}</button><button className="st-btn-ghost" onClick={() => setIssued(null)}>{tr('Ya la he entregado: ocultar', 'Delivered: hide credential')}</button></div></section>}
    <section className="st-panel"><h2>{editing ? tr('Editar ficha', 'Edit profile') : tr('Crear alumno', 'Create student')}</h2><form onSubmit={(event) => { event.preventDefault(); void save() }}>
      ...
      <label>{tr('Identificador de acceso', 'Login identifier')}<input value={draft.login} required minLength={3} maxLength={120} autoComplete="off" pattern="[a-z0-9._@-]+" onChange={(e) => setDraft({ ...draft, login: e.target.value.toLowerCase() })} /></label>
      ...
      <div className="st-actions"><button className="st-btn" disabled={busy || !draft.name.trim() || draft.login.trim().length < 3}>{editing ? tr('Guardar ficha', 'Save profile') : tr('Crear acceso remoto', 'Create remote access')}</button>{editing && <button type="button" className="st-btn-ghost" onClick={() => { setEditing(null); setDraft(EMPTY) }}>{tr('Cancelar edición', 'Cancel editing')}</button>}</div>
    </form></section>
    <section className="st-panel"><h2>{tr('Alumnos', 'Students')} ({learners.length})</h2>{!learners.length && !busy && <p>{tr('No hay alumnos cargados. Crea uno o vuelve a actualizar.', 'No students loaded. Create one or refresh the list.')}</p>}
      {learners.map((learner) => <article key={learner.id} className="st-block"><h3>{learner.name}</h3><p>{learner.login} · {learner.level} · {learner.status}{!learner.enabled ? tr(' · necesita restablecer clave', ' · credential reset required') : ''}{learner.expiresAt ? ` · ${tr('caduca', 'expires')} ${new Date(learner.expiresAt).toLocaleString(locale)}` : ''}</p><p>{learner.goal}</p><div className="st-actions">
