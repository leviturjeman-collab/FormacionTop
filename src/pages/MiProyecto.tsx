...
import { TOOL_PURPOSES } from '../tool-purposes'
          ...
          {prompts.map((prompt) => openId === prompt.id ? (
            <article key={prompt.id} className="st-saved-prompt-detail">
              ...
              <button
                ...
                className="st-btn"
                onClick={() => {
                  void copyText(prompt.prompt).then(() => { setCopied(prompt.id); window.setTimeout(() => setCopied(null), 1600) }).catch(() => window.alert(locale === 'en' ? 'Select the text and copy it manually.' : 'Selecciona el texto y cópialo manualmente.'))
                }}
              >
...
function ToolChoice({ tool, selected, recommended, onClick, locale }: { tool: ToolPage; selected: boolean; recommended: boolean; onClick: () => void; locale: Locale }) {
  return <button type="button" className={`st-tool-choice${selected ? ' selected' : ''}`} onClick={onClick} aria-pressed={selected}>
    <span className="st-tool-choice-mark">{selected ? <Check size={13} /> : <Wrench size={13} />}</span>
    <span><strong>{tool.label}</strong><small className="st-tool-choice-purpose">{TOOL_PURPOSES[tool.id]?.[locale] || (locale === 'en' ? 'Explore its practical guide' : 'Explora su guía práctica')}</small>{recommended && <small className="st-tool-choice-recommended">{locale === 'en' ? 'Recommended' : 'Recomendada'}</small>}</span>
  </button>
}
