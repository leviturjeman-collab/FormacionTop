...
export const store = {
  ...
  enter(profile: { id?: string; name: string; preferredLevel?: LevelId; locale?: 'es' | 'en'; teacher?: boolean; access: 'learner' | 'admin'; project?: ProjectProfile }, server?: unknown, version = 0) {
    ...
    let cached = (profile.id ? volatileRecovery.get(profile.id) : undefined) || persisted
    let incoming = fresh()
    if (record(server) && record(server.lessons)) incoming = validateStudent(server)
    // Recover the former single-browser format only for the SAME verified
    // learner and only when neither a newer local copy nor remote work exists.
    if (!cached && profile.id && !Object.keys(incoming.lessons).length && !incoming.projects.length) {
      try {
        const raw = localStorage.getItem(LEGACY_KEY)
        const legacy = raw ? JSON.parse(raw) : null
        if (record(legacy) && legacy.id === profile.id) cached = { schemaVersion: 2, revision: 0, state: validateStudent(legacy), remoteVersion: version, remoteBase: fresh(), pending: true }
      } catch { /* Keep the original bytes; a damaged copy never overwrites the server. */ }
    }
  ...
  saveQuiz(slug: string, level: LevelId, correct: number, total: number) {
    const previous = state.lessons[slug]?.quiz[level]
    if (previous && previous.correct >= correct) return
    changeLesson(slug, (p) => ({ ...p, quiz: { ...p.quiz, [level]: { correct, total } } }))
  },
