...
export default function PracticeLabView({lab}:{lab:PracticeLab}){
 ...
 return <section className="st-panel st-practice-lab" aria-labelledby={id}>
  ...
  <h2 id={id}>{t('Un caso explicado para que puedas hacerlo tú','An explained case you can do yourself')}</h2>
  ...
  {lab.notes.length>0&&<div className="st-lab-explanation"><h3>{t('Antes de tocar nada: entiende lo que estás haciendo','Before making changes: understand the task')}</h3>{lab.notes.map((p,i)=><p key={i}>{p}</p>)}</div>}
