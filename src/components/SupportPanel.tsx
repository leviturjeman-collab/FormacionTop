  ...
  return <section id="support-panel" className="st-project-workspace">
    <details open={open} onToggle={event => setOpen(event.currentTarget.open)}>
      ...
      <form onSubmit={send}>
        ...
        <button className="st-btn" disabled={busy || session.status !== 'authenticated' || !draft.subject?.trim() || !draft.expected?.trim() || !draft.observed?.trim()}>{busy ? (en ? 'Sending…' : 'Enviando…') : (en ? 'Send question to the teacher' : 'Enviar duda al profesor')}</button>
      </form>
    ...
    <button className="st-btn-ghost" disabled={busy || session.status !== 'authenticated'} onClick={async () => { setBusy(true); setMessage(''); try { await load(); setMessage(en ? 'Requests refreshed.' : 'Consultas actualizadas.') } catch (error) { setMessage(error instanceof Error ? error.message : 'No se pudieron cargar las consultas') } finally { setBusy(false) } }}>{en ? 'View / refresh my questions' : 'Ver / actualizar mis consultas'}</button>
